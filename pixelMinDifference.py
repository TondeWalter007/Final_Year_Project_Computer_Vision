...
img = cv2.imread("images/urchin_cropped3.jpg", cv2.IMREAD_COLOR)
...
tl = Image.new(mode="RGB", size=(width, height), color=(r1, g1, b1))
tl.save("images/tl.jpg")

tr = Image.new(mode="RGB", size=(width, height), color=(r2, g2, b2))
tr.save("images/tr.jpg")

bl = Image.new(mode="RGB", size=(width, height), color=(r3, g3, b3))
bl.save("images/bl.jpg")

br = Image.new(mode="RGB", size=(width, height), color=(r4, g4, b4))
br.save("images/br.jpg")

img_copy = new_img.copy()
tl_jpg = cv2.imread("images/tl.jpg", cv2.IMREAD_COLOR)
tl_jpg = cv2.cvtColor(tl_jpg, cv2.COLOR_BGR2RGB)
tr_jpg = cv2.imread("images/tr.jpg", cv2.IMREAD_COLOR)
tr_jpg = cv2.cvtColor(tr_jpg, cv2.COLOR_BGR2RGB)
bl_jpg = cv2.imread("images/bl.jpg", cv2.IMREAD_COLOR)
bl_jpg = cv2.cvtColor(bl_jpg, cv2.COLOR_BGR2RGB)
br_jpg = cv2.imread("images/br.jpg", cv2.IMREAD_COLOR)
br_jpg = cv2.cvtColor(br_jpg, cv2.COLOR_BGR2RGB)

corners_list = [tl_jpg, tr_jpg, bl_jpg, br_jpg]
corner_labels = ["Top Left Color", "Top Right Color", "Bottom Left Color", "Bottom Right Color"]

fig=plt.figure(figsize=(8, 8))
columns = 2
rows = 2
for i in range(1, columns*rows +1):
    img = corners_list[i-1]
    fig.add_subplot(rows, columns, i)
    plt.imshow(img)
    plt.title(corner_labels[i-1])
plt.show()


hsv1 = cv2.cvtColor(img_copy, cv2.COLOR_RGB2HSV)
hsv_tl = cv2.cvtColor(tl_jpg, cv2.COLOR_RGB2HSV)
hsv_tr = cv2.cvtColor(tr_jpg, cv2.COLOR_RGB2HSV)
hsv_bl = cv2.cvtColor(bl_jpg, cv2.COLOR_RGB2HSV)
hsv_br = cv2.cvtColor(br_jpg, cv2.COLOR_RGB2HSV)

h1, s1, v1 = hsv1[0, 50]
h0, s0, v0 = hsv_tl[int(height / 2), int(width / 2)]
...
for y in range(0, height):
    for x in range(0, width):
        ...
        h0, s0, v0 = hsv_tr[int(height / 2), int(width / 2)]
        ...
        h0, s0, v0 = hsv_bl[int(height / 2), int(width / 2)]
        ...
        h0, s0, v0 = hsv_br[int(height / 2), int(width / 2)]
...
print("Last Min Distance:", min_distance)
plt.imshow(img_copy)
